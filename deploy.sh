...
sudo docker build -f "Dockerfile" -t setyoramdhoni07/docker_creartmart:frontend-$TRAVIS_BUILD_NUMBER  --label "Maintaner  Setyo Ramdhoni <setyo@alterra.id>" .
# push apps image to docker hub
sudo docker push setyoramdhoni07/docker_creartmart:frontend-$TRAVIS_BUILD_NUMBER
...
kubectl -n alta17 --kubeconfig kubeconfig set image deployment/${APP_K8S_DEPLOYMENT_NAME} ${APP_K8S_CONTAINER_NAME}=setyoramdhoni07/docker_creartmart:frontend-${TRAVIS_BUILD_NUMBER}